...
class Task extends HTMLElement {
  connectedCallback() {
    injectCSS('task-card', 'task.css', import.meta.url);
    ...
    checkbox.addEventListener('change', async () => {
      ...
      try {
        const res = await fetch(`/tasks/api/tasks/${taskId}/toggle`, {
          method: 'POST',
    ...
    if (!isShared) {
      ...
      moveConfirm.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch(`/tasks/api/tasks/${taskId}/move`, {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    deleteBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await fetch(`/tasks/api/tasks/${taskId}`, { method: 'DELETE' });
        if (!res.ok) throw new Error();
