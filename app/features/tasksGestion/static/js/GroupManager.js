...
class GroupManager extends HTMLElement {
  connectedCallback() {
    injectCSS('group-manager', 'groupManager.css', import.meta.url);
    this._groups      = [];
    this._users       = [];
  ...
  async _doCreate(input, colorPicker) {
    ...
    try {
      const res  = await fetch('/tasks/api/groups', {
        method:  'POST',
  ...
  async _doShare(groupId, userId, username, taskUserEl) {
    try {
      const res = await fetch(`/tasks/api/groups/${groupId}/share`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async _doDelete(groupId, groupName) {
    ...
    try {
      const res = await fetch(`/tasks/api/groups/${groupId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
  ...
  async _doLeave(groupId, groupName) {
    ...
    try {
      const res = await fetch(`/tasks/api/groups/${groupId}/leave`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
      this._groups = this._groups.filter(g => String(g.id) !== String(groupId));
  ...
  async _doRevoke(groupId, userId, username, taskUserEl) {
    try {
      const res = await fetch(`/tasks/api/groups/${groupId}/share/${userId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
