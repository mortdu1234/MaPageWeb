...
proxmox_bp = make_blueprint("proxmox", __name__, __file__, url_prefix="/dashboard/proxmox")

@proxmox_bp.route("/api/debug")
...
@require_permission("admin")
def proxmox_page():
    return render_template(proxmox_bp.get_templates_path("dashboard.html"))

@proxmox_bp.route("/api/stats")
@require_permission("admin")
def proxmox_stats():
    try:
        cfg  = _cfg()
