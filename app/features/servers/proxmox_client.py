...
_net_cache = {"timestamp": 0.0, "counters": None}
...
def _get_vms():
    try:
        ...
        if token and value:
            prox = ProxmoxAPI(host, user=user, token_name=token, token_value=value, verify_ssl=verify)
        else:
            password = os.environ.get("PROXMOX_PASSWORD", "")
            prox = ProxmoxAPI(host, user=user, password=password, verify_ssl=verify)

        vms = []
        for item in prox.nodes(node).lxc.get():
            vms.append({"vmid": item["vmid"], "name": item.get("name", f"CT {item['vmid']}"), "status": item.get("status", "unknown"), "type": "LXC"})
        for item in prox.nodes(node).qemu.get():
            vms.append({"vmid": item["vmid"], "name": item.get("name", f"VM {item['vmid']}"), "status": item.get("status", "unknown"), "type": "QEMU"})
...
__all__ = ["_get_net_rates", "_get_temperatures", "_get_disks", "_get_vms", "_get_cpu_model", "_get_ip"]
