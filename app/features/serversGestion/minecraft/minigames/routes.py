...
Page HTML :
  GET  /server/minecraft/minigames/              -> affiche la page de gestion

API :
  GET  /server/minecraft/minigames/api/liste      -> liste des minijeux + minijeu actif
  POST /server/minecraft/minigames/api/switch     -> stop -> switch (SSH) -> start,
...
from app.core.utils.PterodactylClient import PterodactylClientWrapper

serverMinijeux_bp = make_blueprint("serverMinijeux", __name__, __file__, url_prefix="/server/minecraft/minigames")
...
_CURRENT_GAME_FILE = os.path.join("data", "json", "current_minigame.json")
...
def _get_server_state() -> str | None:
    ...
    utilization = PterodactylClientWrapper.get_utilization(Config.MINIGAME_VM_PTERODACTYL_ID)
...
def serveur_minijeux_page():
    ...
    return render_template(serverMinijeux_bp.get_templates_path("minigames.html"))


@serverMinijeux_bp.route("/api/liste")
...
@serverMinijeux_bp.route("/api/switch", methods=["POST"])
def api_switch_minijeu():
    ...
    def generate():
        try:
            ...
            PterodactylClientWrapper.send_power_action(Config.MINIGAME_VM_PTERODACTYL_ID, "stop")
            ...
            PterodactylClientWrapper.send_power_action(Config.MINIGAME_VM_PTERODACTYL_ID, "start")
